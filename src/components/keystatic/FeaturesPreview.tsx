...
type FeaturesState = { title: string; features: FeatureItem[] };
...
function FeaturesInput({ value, onChange }: { value: string, onChange: (v: string) => void }) {
    ...
    const updateFeature = (index: number, key: keyof FeatureItem, val: string) => {
        ...
        const next = { ...state, features: nextFeatures };
        setState(next);
        onChange(JSON.stringify(next));
    };

    const addFeature = () => {
        const next = { ...state, features: [...state.features, { title: 'Nueva Ventaja', description: 'Detalle de la ventaja...', icon: 'CheckCircle' }] };
        setState(next);
        onChange(JSON.stringify(next));
    };

    const removeFeature = (idx: number) => {
        const next = { ...state, features: state.features.filter((_, i) => i !== idx) };
        setState(next);
        onChange(JSON.stringify(next));
    };
        ...
        <FieldPrimitive label="💎 Ventajas/Características (Vista Previa)">
            ...
            <div style={{ background: '#0f172a', border: '1px solid rgba(239,68,68,0.15)', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: '24px' }}>
                <div style={{ marginBottom: '20px' }}>
                    <div style={{ fontSize: '10px', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: '5px' }}>Título de Sección</div>
                    <input style={inp} value={state.title} onChange={e => updateTitle(e.target.value)} />
                </div>
